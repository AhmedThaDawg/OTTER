...
int main()
{
	...
	static const float triangle[] = {
		// X    Y     Z     R     G     B
		0.5f, -0.5f, 0.5f, 0.2f, 0.4f, 0.3f, 
		0.5f, 0.5f, 0.5f, 0.5f, 0.5f, 0.3f,  
		-0.5f, 0.5f, 0.5f, 0.2f, 0.0f, 0.2f, 
		
	};

	VertexBuffer* triangle_vbo = new VertexBuffer();
	triangle_vbo->LoadData(triangle, 6 * 4);
	...
	while (!glfwWindowShouldClose(window)) {
		...
		vao3->Bind();
		glDrawElements(GL_TRIANGLES,
			(GLenum)triangle_ibo->GetElementCount(),
			(GLenum)triangle_ibo->GetElementType(), nullptr);
		VertexArrayObject::Unbind();
